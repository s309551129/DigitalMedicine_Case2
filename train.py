...
from utils import CenterLoss, AverageMeter, TopKAccuracyMetric, batch_augment, split_data
...
def collect_data():
    ...
    train_info, val_info = split_data("./data/data_info.csv", 20)
    train_dataset = Dataset(root="./data/train/", info=train_info, img_size=args.img_size, mode="train")
    valid_dataset = Dataset(root="./data/train/", info=val_info, img_size=args.img_size, mode="valid")
    train_loader = DataLoader(train_dataset, batch_size=args.batch_size, shuffle=True, num_workers=4, pin_memory=True)
    valid_loader = DataLoader(valid_dataset, batch_size=args.batch_size*4, shuffle=False, num_workers=4, pin_memory=True)
...
def run():
    ...
    for epoch in range(args.epoch):
        ...
        valid_acc = validate(data_loader=valid_loader, net=net, pbar=pbar, train_info=train_info)
...
if __name__ == '__main__':
    ...
    print(args.net)
    train_loader, valid_loader = collect_data()
    
    # Initialize model
    net = WSDAN(M=args.num_attentions, net=args.net, pretrained=True)
    # feature_center: size of (#classes, #attention_maps * #channel_features)
    feature_center = torch.zeros(3, args.num_attentions * net.num_features).to(args.device)
